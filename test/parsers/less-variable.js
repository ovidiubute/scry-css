...
describe('LESS Variable parser', function () {
  describe('#parse', function () {
    it('should parse variable with a hex color value', function () {
      expect(LessVariableParser.parse('@link-color:        #428bca;'))
          .to.deep.equal({
    ...
    it('should parse variable with a text color value', function () {
      expect(LessVariableParser.parse('@some-color: red;')).to.deep.equal({
        variable: 'some-color', value: 'red'
      })
    })

    it('should parse variable with a rgba color value', function () {
      expect(LessVariableParser.parse('@widget-b-color:     rgba(0,0,0,255);'))
          .to.deep.equal({
    ...
    it('should parse variable with a px value', function () {
      expect(LessVariableParser.parse('@normal-line-height:   12px;'))
          .to.deep.equal({
    ...
    it('should parse variable with a percentage value', function () {
      expect(LessVariableParser.parse('@widget-min-width: 50%;'))
          .to.deep.equal({
    ...
    it('should parse variable with a text value', function () {
      expect(LessVariableParser.parse('@widget-align: normal;')).to.deep.equal({
        variable: 'widget-align', value: 'normal'
      })
    })

    it('should not parse a non variable declaration line', function () {
      expect(LessVariableParser.parse('.icon-a {')).to.equal(null)
    })
  })
})
